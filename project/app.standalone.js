...
(() => {
  ...
  function renderDots(year) {
    ...
    if ((state.dataset === 'people' || state.dataset === 'both') && state.people) {
      for (const p of state.people) {
        ...
        if (!pt) continue;
        const r = Math.max(1.5, (p.hpi - 60) / 10) * dotScale;
  ...
  function renderLabels(year, t) {
    ...
    if (state.dataset === 'people' || state.dataset === 'both') {
      const hpiMin = k > 5 ? 70 : (k > 3.5 ? 80 : 88);
  ...
  function selectItem(d, type) {
    ...
    if (type === 'city') {
      ...
      const pips = document.getElementById('card-age-bars');
      pips.innerHTML = '';
      const filled = Math.round(d.hpi/10);
  ...
  function buildTimeline() {
    ...
    for (const ty of [-10000,-8000,-6000,-4000,-2000,0,500,1000,1500,2000]) {
      const el = document.createElement('div');
      el.className = 'tick';
      el.style.left = ((ty-YEAR_MIN)/(YEAR_MAX-YEAR_MIN)*100)+'%';
  ...
  function playLoop(t) {
    ...
    if (ny >= YEAR_MAX) {
      ny = YEAR_MAX; state.playing = false;
